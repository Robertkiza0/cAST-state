"""Passe de test du retriever pondéré+attention AST sur des chunks produits
par la vraie librairie cAST (astchunk, split-then-merge) au lieu des fenêtres
glissantes (ast_chunker) ou des conteneurs faits maison
(container_ast_chunker), sur les 50 tâches officielles RepoCoder."""
...
from cast_pipeline import (
    load_and_chunk_repo_cast_cached,
    retrieve_top_k_weighted_cast,
)
...
def run_demo(n_tasks: int = 50, tasks_per_repo: int = 10) -> None:
    ...
    for task in tasks:
        ...
        if repo not in chunk_cache:
            print(f"\nPréparation de {repo} (découpage cAST, IDF)...")
            chunk_cache[repo] = load_and_chunk_repo_cast_cached(str(repo_dir))
            doc_weights_cache[repo] = compute_doc_weights(chunk_cache[repo])
            print(f"  -> {len(chunk_cache[repo])} chunks cAST, {len(doc_weights_cache[repo])} symboles distincts")
        ...
        results = retrieve_top_k_weighted_cast(
