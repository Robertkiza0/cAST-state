...
class ASTChunkBuilder():
    ...
    def convert_windows_to_chunks(self, ast_windows: list[list[ASTNode]], 
        ...
        # Shared for this chunkify() call only: many chunks nested in the same
        # large class/function ancestor would otherwise each re-walk that
        # ancestor's whole subtree to recompute its cAST-Scope annotation
        # (self.* state / decorators). Node identity is stable for as long as
        # this tree is alive, i.e. for this call, so caching by id() here is safe.
        ancestor_annotation_cache: dict[int, str] = {}
        ...
        for current_window in ast_windows:
            current_chunk = ASTChunk(
                ...
                ancestor_annotation_cache=ancestor_annotation_cache,
            )
